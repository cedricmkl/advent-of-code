...
function doesOverlap(pair: [Assingnment, Assingnment]): boolean {
    const pair1Nums = range(pair[0][0], pair[0][1])
    const pair2Nums = range(pair[1][0], pair[1][1])
    return pair1Nums.filter((i) => !pair2Nums.includes(i)).length !== pair1Nums.length 
        || pair2Nums.filter((i) => !pair1Nums.includes(i)).length !== pair2Nums.length
}
...
console.log(assingmentPairs.filter((pair) => doesOverlap(pair)).length)
